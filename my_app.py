...
def merchant_login():
    url_merchant_login = "endpoint for merchant login"
...
def get_transaction_report():
    url_transaction_report = "endpoint for transaction REPORT info"
    ...
    from_date_full = datetime.strptime(to_date, '%Y-%m-%d')
    ...
    to_date_full = datetime.strptime(to_date, '%Y-%m-%d') 
...
@app.route('/client')
def photos():
    url_for_client = "endpoint for client info"
    headers = {"Authorization": token}
